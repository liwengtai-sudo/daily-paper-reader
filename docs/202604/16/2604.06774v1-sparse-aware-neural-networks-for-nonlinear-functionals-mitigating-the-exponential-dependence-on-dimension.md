...
tags: ["query:dl"]
score: 6.0
evidence: 在函数式学习中提取稀疏特征的卷积架构
tldr: 针对算子学习中非线性泛函面临的维度灾难和可解释性难题，本文提出了一种稀疏感知神经网络框架。该框架结合卷积架构提取稀疏特征与深度全连接网络进行函数逼近，利用通用离散化方法实现了从有限样本中的稳定恢复。研究证明，通过利用函数空间的稀疏性，可以显著提高逼近速率并减少所需样本量，为缓解泛函学习中的维度指数依赖提供了理论支持。
source: arxiv
...
motivation: 解决现有算子学习理论在处理高维函数空间时面临的维度灾难及模型可解释性不足的问题。
method: 提出一种结合卷积神经网络提取稀疏特征与深度全连接网络进行非线性泛函逼近的混合架构。
result: 在具有快速频率衰减和混合平滑性的函数空间中，实现了更优的逼近速率和更小的样本需求。
conclusion: 稀疏性是缓解泛函学习中维度灾难的关键，为高维非线性泛函的稳定恢复提供了新的理论视角。
---
