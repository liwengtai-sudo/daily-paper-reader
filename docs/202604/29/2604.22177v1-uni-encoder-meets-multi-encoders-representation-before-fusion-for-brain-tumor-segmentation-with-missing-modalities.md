...
title_zh: Uni-Encoder 遇上 Multi-Encoders：模态缺失下脑肿瘤分割的先表示后融合方法
...
tags: ["query:ai-pm"]
score: 9.0
evidence: 利用多模态MRI影像进行脑肿瘤分割的深度学习研究
tldr: 提出一种两阶段ViT架构，旨在解决MRI模态缺失情况下的脑肿瘤分割性能下降问题。
source: arxiv
...
motivation: 利用多模态MRI影像进行脑肿瘤分割的深度学习研究。
...
## 摘要
多模态 MRI 为脑肿瘤分割提供了互补信息，但临床扫描通常缺失一个或多个模态，这会导致分割性能下降。在本文中，我们提出了 UniME（Uni-Encoder Meets Multi-Encoders），这是一种用于模态缺失下脑肿瘤分割的两阶段异构方法，旨在权衡细粒度结构捕捉、跨模态互补性建模以及可用模态利用之间的关系。其核心思想是通过两阶段异构架构将表示学习与分割解耦。第一阶段通过掩码图像建模预训练单个 ViT Uni-Encoder，以建立对缺失模态具有鲁棒性的统一表示。第二阶段添加了特定模态的 CNN Multi-Encoders，以提取高分辨率、多尺度、细粒度的特征。我们将这些特征与全局表示融合，以生成精确的分割结果。在 BraTS 2023 和 BraTS 2024 上的实验表明，UniME 在不完整多模态场景下优于以往的方法。代码可在 https://github.com/Hooorace-S/UniME 获取。
